...
{
    public class CdHelper_db
    {

        /// <summary>
        /// Adds a new instance into the database.
        /// </summary>
        public static Cd_db Add(int systemId, string title, string libraryAddress, string genre, DateTime publishingDate, int authorId,
            int? borrowerId, DateTime? dateOfCheckOut, DateTime? dueDate, int length, DbContext context, out StatusResponse statusResponse)
        {
            try
            {
                // Validate
                ...
                Cd_db instance = new Cd_db
                    (
                        systemId: 0, //This can be ignored is PK in your DB is auto increment
                        title, libraryAddress, genre, publishingDate, authorId, borrowerId, dateOfCheckOut, dueDate, length
                    );
        ...
        /// </summary>
        public static Cd_db Edit(int systemId, string title, string libraryAddress, string genre, DateTime publishingDate, int authorId,
           ...
           DbContext context, out StatusResponse statusResponse)
        {
            try
            {
                ...
                rowsAffected = context.ExecuteNonQueryCommand
                (
                    commandText: "UPDATE cd SET length = @length WHERE system_id = @system_id",
                    parameters: new Dictionary<string, object>()
                    {
                                        {"@system_id", instance.System_id },
                                        { "@length", instance.Length }
        ...
        public static void Delete(int systemId, DbContext context, out StatusResponse statusResponse)
        {
            try
            {
                ...
                int rowsAffected = context.ExecuteNonQueryCommand
                    (
                        commandText: "DELETE FROM cd WHERE system_id = @system_id",
                        parameters: new Dictionary<string, object>()
                        {
                            {"@system_id", systemId },
                        },
        ...
        /// </summary>
        public static Cd_db Get(int systemId,
            DbContext context, out StatusResponse statusResponse)
        {
            try
            {
                ...
                table = context.ExecuteDataQueryCommand
                    (
                        commandText: "SELECT * FROM cd WHERE system_id = @system_id",
                        parameters: new Dictionary<string, object>()
                        {
                            {"@system_id", systemId },
                        },
                ...
                // Return value
                statusResponse = new StatusResponse("Cd has been retrieved successfully.");
                return instance;
            }
            catch (Exception exception)
            {
        ...
        /// </summary>
        public static List<Cd_db> GetCollection(
            DbContext context, out StatusResponse statusResponse)
        {
            try
            {
